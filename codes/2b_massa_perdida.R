...
for(aa in ano) {
  for (tri in trimestre) {
    ...
    modelo_salarial <- feols(ln_r_hab_all  ~ male +.[var_x], 
    ...
    e_wg_mulher <-  e_wg_homem + Delta_X_beta + gama
  
    
    # Empregabilidade
    modelo_empregabilidade <- feols(emp ~ male +  .[var_z], 
                                    data = dt, weights = ~V1028, fsplit = ~ female)
    ...
    teta <- as.matrix(as.numeric(matrix_phi[1,4]))
    ...
    p_emp_homem <- teta + (matrix_z_homem%*%phi)
    ...
    massa_perdida_emp_equivalente <- massa_perdida_mulher - salario_perdido_mulher
