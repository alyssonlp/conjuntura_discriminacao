...
frac_r_efe_all <- dt1 %>%
  ...
  ggplot(aes(x = interaction(Ano_trimestre, gender_race), 
             y = frac_renda_media_efe, fill = gender_race)) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = round(frac_renda_media_efe)),
            vjust = -0.5, position = position_dodge(width = 0.8), size = 8) +
...
frac_r_hab_all <- dt1 %>%
  filter(gender_race %in% c("Mulher Branca", "Homem Negro", "Mulher Negra" )) %>%
  ggplot(aes(x = interaction(Ano_trimestre, gender_race),
             y = frac_renda_media_hab, fill = gender_race)) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = round(frac_renda_media_hab)),
            vjust = -0.5, position = position_dodge(width = 0.8), size = 8) +
  scale_fill_manual(name = "",
                    values = c("Mulher Branca" = "darkorange1",
                               "Homem Negro" = "darkgoldenrod1",
...
frac_unemp <- dt1 %>%
  filter(gender_race %in% c("Mulher Branca", "Homem Negro", "Mulher Negra" )) %>%
  ggplot(aes(x = interaction(Ano_trimestre, gender_race),
             y = frac_unemp, fill = gender_race)) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = round(frac_unemp,2)), 
            vjust = -0.5, position = position_dodge(width = 0.8), size = 8) +
  scale_fill_manual(name = "",
                    values = c("Mulher Branca" = "darkorange1",
                               "Homem Negro" = "darkgoldenrod1",
...
frac_pea <- dt1 %>%
  filter(gender_race %in% c("Mulher Branca", "Homem Negro", "Mulher Negra" )) %>%
  ggplot(aes(x = interaction(Ano_trimestre, gender_race),
             y = frac_pea, fill = gender_race)) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = round(frac_pea,2)), vjust = -0.5,
...
frac_massa_efe <- dt1 %>%
  filter(gender_race %in% c("Mulher Branca", "Homem Negro", "Mulher Negra" )) %>%
  ggplot(aes(x = interaction(Ano_trimestre, gender_race),
             y = frac_massa_efe, fill = gender_race)) +
  geom_bar(stat = 'identity', position = position_dodge(width = 0.8), width = 0.7) +
  geom_text(aes(label = round(frac_massa_efe)), vjust = -0.5, 
            position = position_dodge(width = 0.8), size = 5) +
  scale_fill_manual(name = "",
                    values = c("Mulher Branca" = "darkorange1",
                               "Homem Negro" = "darkgoldenrod1",
