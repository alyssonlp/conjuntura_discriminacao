...
pdf(file.path(figures_output, "base_topo_1_linha.pdf"),  width = 14, height = 8.5)
um_porcento <- one %>% 
  ggplot(aes(x = Ano_trimestre, y = prop, group = gender_race, color = gender_race)) + 
  geom_line(size = 1.6) +
...
pdf(file.path(figures_output, "base_topo_5_linha.pdf"),  width = 14, height = 8.5)
cinco <- five %>% 
  ggplot(aes(x = Ano_trimestre, y = prop, group = gender_race, color = gender_race)) + 
  geom_line(size = 1.6) +
  scale_color_manual(name = "", 
...
pdf(file.path(figures_output, "base_topo_10_linha.pdf"),  width = 14, height = 8.5)
dez <- ten %>% 
  ggplot(aes(x = Ano_trimestre, y = prop, group = gender_race, color = gender_race)) + 
  geom_line(size = 1.6) +
  scale_color_manual(name = "", 
