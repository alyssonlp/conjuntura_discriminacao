...
pdf(file.path(figures_output, "base_topo_1_ppb.pdf"),  width = 14, height = 8.5)
um_porcento <- one %>% 
  ggplot() + 
  geom_area(aes(x = Ano_trimestre, y = prop, group = raca, fill = raca)) +
...
pdf(file.path(figures_output, "base_topo_5_ppb.pdf"),  width = 14, height = 8.5)
cinco <- five %>% 
  ggplot() + 
  geom_area(aes(x = Ano_trimestre, y = prop, group = raca, fill = raca)) +
  scale_color_manual(name = "", 
...
pdf(file.path(figures_output, "base_topo_10_ppb.pdf"),  width = 14, height = 8.5)
dez <- ten %>% 
  ggplot() + 
  geom_area(aes(x = Ano_trimestre, y = prop, group = raca, fill = raca)) +
  scale_color_manual(name = "", 
