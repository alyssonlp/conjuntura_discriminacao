...
for(aa in ano) {
  for (tri in trimestre) {
    ...
    teste4 <- wtd.mean(dt[male == 1 & nonwhite == 0, r_hab_all],
                       weights = dt[male == 1 & nonwhite == 0, V1028], na.rm = T)
    ...
    eq_emp_h <- lm(pea_emp ~ nonwhite + factor(age_group)*parents + 
                     factor(age_group)*educ + factor(age_group)*educ_sq + 
                     factor(age_group)*potential_xp + factor(age_group)*potential_xp_sq + 
                     factor(age_group)*married +
                     factor(age_group)*rural + factor(age_group)*metropolitan + 
                     factor(age_group)*factor(estudando) + RO + 
    ...
    media_emp_hn_teste <- wtd.mean(dt$pea_emp[pos_hn_emp],
                                   weights = dt$V1028[pos_hn_emp])
    ...
    eq_emp_m <- lm(pea_emp ~ nonwhite + parents + 
                     educ + educ_sq + 
                     potential_xp + potential_xp_sq + 
                     married +
                     rural + metropolitan + 
                     estudando + RO +  
                     AC + AM + RR + PA + AP + TO + MA + PI + CE + RN + PB + PE + AL + 
                     SE + BA + MG + ES + RJ + SP + PR + SC + RS + MS + MT + GO, 
                   weights = V1028, data = dt[male == 0 & V2009 >= 25 & V2009 <= 60,])
    
    pos_mn_emp <- which(dt[male == 0, nonwhite == 1])
    pos_mb_emp <- which(dt[male == 0, nonwhite == 0])
    
    dem <- sum(dt[, r_hab_all*V1028], na.rm = T)/1000000000
    num <- sum(dt[V2009 <= 60 & V2009 >= 25, r_hab_all*V1028], na.rm = T)/1000000000
    num/dem
